...
corpus_name: str | list[str] | tuple[str, ...] = (
    "brown",
    "gutenberg",
    "reuters",
    "inaugural",
...
def analyze_corpus(
    tokenized: Sequence[str], shuffle: bool = True
) -> AdvancedTools:
    """
...
def basic_analysis(corpus_tools: CorpusTools) -> None:
    ...
    logger.info(
        f"• Median Token:         '{median['token']}' (Frequency: {median['frequency']})"
    )
    logger.info(
        f"• Mean Token Frequency: {corpus_tools.mean_token_frequency():.2f}"
    )
    ...
    for tk in corpus_tools.list_tokens_in_rank_range(1, 5):
        logger.info(
            f"• Rank {tk['rank']:<2}: '{tk['token']}' (Frequency: {tk['frequency']})"
        )
    ...
    try:
        ...
        logger.info(
            f"• Info for '{example_token}': Frequency: {info['frequency']}, Rank: {info['rank']}"
        )
...
def advanced_analysis(advanced_tools: AdvancedTools) -> None:
    ...
    heaps_result = advanced_tools.calculate_heaps_law()
    if heaps_result is None:
        logger.warning("Heaps' Law fitting failed — skipping.")
        return
    K, beta = heaps_result
    ...
    diff_pct = (
        abs((actual_vocab - est_vocab) / actual_vocab) * 100
        if actual_vocab
        else 0.0
    )
    ...
    logger.info(SUB_SHORT)
    logger.info(
        f"• Zipf's Law Alpha:     {advanced_tools.calculate_zipf_alpha():.2f}"
    )
...
def entropy_metrics(entropy_calculator: EntropyCalculator) -> None:
    ...
    logger.info(f"\n• H3: {H3:.2f} bits")
    logger.info(
        f"   - {entropy_calculator.q_grams}-gram model, captures sub-linguistic patterns"
    )
...
def generate_plots(
    ...
    for plot_id in plots_to_generate:
        if plot_id in plot_map:
            plot_map[plot_id]()
            logger.info(
                f"• Generated {plot_id.replace('_', ' ').title()} plot."
            )
        else:
