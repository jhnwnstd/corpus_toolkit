...
### CorpusTools
Frequency distribution, rank queries, cumulative frequency analysis, hapax/dis legomena, median token.
...
| `calculate_heaps_law()` | Heaps' Law params (K, beta) via weighted curve fitting |
...
Zipf fitting uses Heaps' beta as an informed initial guess (beta ≈ 1/alpha relationship). All results are cached after first computation.
...
| H3 | `calculate_H3_kenlm()` | KenLM n-gram cross-entropy (log10→bits), trained on continuous text |
| R | `calculate_redundancy(H3, H0)` | (1 - H3/H0) × 100% |

Guaranteed ordering: H0 ≥ H1 ≥ H2 ≥ H3. Results cached.

### CorpusPlots
Saves Zipf, Zipf-Mandelbrot, and Heaps' Law plots to `plots/`.

## Sample Output (Brown Corpus)

```
Zipf alpha:          1.02
Zipf-Mandelbrot:     q=1.27, s=1.08
Heaps' Law:          K=64.92, beta=0.48
Yule's K:            97.20
Herdan's C:          0.78
H0: 4.70  H1: 4.17  H2: 3.93  H3: 2.12
Redundancy:          54.8%
```
...
```
nltk, numpy, matplotlib, scipy, regex, numba
```
